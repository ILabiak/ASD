    ...
    while (numberOfVisited < n - 1) {
        ...
        if (visited[minTo] == 0) {
            ...
            for (int j = 0; j < n; j++) {
                ...
                // сброс, щоб шлях не порівнювалась з іншими
            nd[minId].weight = 0;
            nd[minId].from = 0;
            nd[minId].to = 0;
        }
...
void paintWeight(HDC hdc, int matrix[n][n], int nx[n], int ny[n], int start, int end){
    ...
    if (matrix[start][end]>0) {
        ...
        if (start == end) {
            ...
            if ((ny[start] == ny[end]) && abs(nx[start] - nx[end]) > 200 ) {
          if(ny[start]>350){
           TextOutA(hdc, ((nx[end]+nx[start])/2)-15 , ny[end] +40, text, matrix[start][end] < 100 ? 2 : 3);
          }else{
          TextOutA(hdc, ((nx[end]+nx[start])/2)+15 , ny[end] -60, text, matrix[start][end] < 100 ? 2 : 3);
          }
            }else if(nx[start] == nx[end]&& abs(ny[start] - ny[end]) > 200){
    if(start >=9 || end >=9){
      TextOutA(hdc, nx[end]-50 , (ny[end] + ny[start])/2, text, matrix[start][end] < 100 ? 2 : 3);
        ...
        if(start>end){
    // Arc(hdc, nx[end]-60, ny[end], nx[start]+60, ny[start], nx[start], ny[start], nx[end], ny[end]);     //hZZZZZZZZZZ
      TextOutA(hdc, ((nx[end]+nx[start])/2)+15 , ny[end] -60, text, matrix[start][end] < 100 ? 2 : 3);
        }else{
      TextOutA(hdc, nx[end]+50 , (ny[end] + ny[start])/2, text, matrix[start][end] < 100 ? 2 : 3);
        }
    ...
    TextOutA(hdc, nx[end]-10 , (ny[end] + ny[start])/2, text, matrix[start][end] < 100 ? 2 : 3);
           ...
           if(matrix[start][end] < 10){
             TextOutA(hdc, ((nx[end]+nx[start])/2)-10 , ((ny[end] + ny[start])/2)-10, text, 1);
               ...
               if(start ==1 && end == 9){
              TextOutA(hdc, ((nx[end]+nx[start])/2)-20 , ((ny[end] + ny[start])/2)+20, text, matrix[start][end] < 100 ? 2 : 3);
               }else if(start == 0 && end == 7){
              TextOutA(hdc, ((nx[end]+nx[start])/2)-20 , ((ny[end] + ny[start])/2)-30, text, matrix[start][end] < 100 ? 2 : 3);
               }else if(start == 0 && end == 6){
              TextOutA(hdc, ((nx[end]+nx[start])/2)-20 , ((ny[end] + ny[start])/2)-30, text, matrix[start][end] < 100 ? 2 : 3);
               }else if(start == 2 && end == 8){
              TextOutA(hdc, ((nx[end]+nx[start])/2), ((ny[end] + ny[start])/2)-40, text, matrix[start][end] < 100 ? 2 : 3);
               }else if(start == 3 && end == 7){
              TextOutA(hdc, ((nx[end]+nx[start])/2)+5, ((ny[end] + ny[start])/2)-50, text, matrix[start][end] < 100 ? 2 : 3);
               }else if(start == 1 && end == 6){
              TextOutA(hdc, ((nx[end]+nx[start])/2)+5, ((ny[end] + ny[start])/2)+20, text, matrix[start][end] < 100 ? 2 : 3);
               }else if((start == 2 && end == 7)||(start == 7 && end == 2)){
              TextOutA(hdc, ((nx[end]+nx[start])/2)-15, ((ny[end] + ny[start])/2)-40, text, matrix[start][end] < 100 ? 2 : 3);
               }
               else{
              TextOutA(hdc, ((nx[end]+nx[start])/2)-10 , ((ny[end] + ny[start])/2)-10, text, matrix[start][end] < 100 ? 2 : 3);
                        }
                    }
                }
            }
...
void printMatrix(HDC hdc, int A[n][n],int xPos) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            ...
            TextOutA(hdc, xPos + 30 * j, 600 + 20 * i, text, A[i][j] < 100 ? 2 : 3);
        }
    }
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT:
        ...
        TextOutA(hdc, 30, 570, "Weight matrix:", 14);
